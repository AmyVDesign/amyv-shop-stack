...
/**
 * ONE continuous shrink-wrap surface sealed over the LED section and up
 * over the whole cable coil to its open mouth, chip bumps, a strip ridge,
 * wrinkle noise fading in from both cut edges (unlike the whip, which only
 * fades in from the bottom, the staff's membrane has no dome cap sealing
 * its top, so both ends are open cuts), coil bulges, and a shrunk-tight
 * taper at both ends. Own copy of the whip's membrane builder, sample
 * density is driven off the tightest pitch actually present (strip vs.
 * coil), not a fixed absolute count, or slimming S quietly under-samples
 * the displacement and the bumps stop forming.
 */
export default function Membrane({ dims, isOff }: MembraneProps) {
  const { y0, span, chipArc, topExtY } = dims;
  ...
  const { geometry, filmMid } = useMemo(() => {
    ...
    const RAD_SEG = 72;
    const finest = Math.min(PITCH, COIL_PITCH);
    ...
    for (let i = 0; i < p.count; i++) {
      ...
      // extended region: film seals over the cable coil, bulging over each
      // coil turn like the macro photos
      const nC = (COIL_TOP - wy) / COIL_PITCH;
      if (nC > -0.5 && nC < COIL_TURNS + 0.5) {
        let cf = (COIL_DIR * nC - th / (Math.PI * 2)) % 1;
        if (cf < 0) cf += 1;
        const dC = Math.min(cf, 1 - cf) * COIL_PITCH;
        const envC = smoothstep((nC + 0.25) / 0.5) * smoothstep((COIL_TURNS + 0.25 - nC) / 0.5);
