...
-- Atomically claims qty units. Returns qty on success, 0 if insufficient stock.
-- All-or-nothing per product: never partial, never negative.
...
BEGIN
  ...
  RETURN rows_updated * qty;
END;
$$;

COMMENT ON FUNCTION claim_parts(uuid, int) IS
  'Atomically claims qty units for checkout. Returns qty on success, 0 on insufficient stock. All-or-nothing, never partial. Service-role only; called exclusively by the server checkout route.';
...
COMMENT ON FUNCTION release_parts(uuid, int) IS
  'Releases a hold created by claim_parts. Called on checkout error or timeout. Service-role only; called exclusively by the server checkout route. Phase 2 webhooks will handle session expiry.';
