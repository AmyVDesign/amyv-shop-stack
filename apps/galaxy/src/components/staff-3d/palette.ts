/**
 * Every material color and canvas-drawing color used by the staff model,
 * collected in one place. Three.js material colors are numeric literals
 * (0xRRGGBB) and canvas 2D colors are CSS hex strings, neither is a design
 * token, since neither renders through CSS. This module is the deliberate,
 * documented exception to "no hardcoded hex in .tsx": every color the staff
 * uses lives here, named, instead of scattered as inline literals through
 * the geometry-building code. See docs/STAFF_MODEL.md.
 *
 * Own copy, not shared with whip-3d/palette (whip-3d has none yet; these
 * two products' materials were independently tuned against different macro
 * photos and are not guaranteed to match numerically even where they look
 * similar, e.g. the staff's cream pole vs. the whip's near-black one).
 */
...
// Foot
export const FOOT_RUBBER = 0x0c0c0e;
export const FOOT_GROOVE = 0x060607;
